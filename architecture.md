...
* An **S3-compatible object storage bucket** — holds report PDFs that are sent for digital signing, and images uploaded through the rich-text editor. Required for the PDF signing flow and image upload to work; there is no filesystem fallback.
...
| REST API clients | Inbound | Lets other municipal systems read and write assets, documents and suppliers, and read organisations and users. See the [API reference](api/api.md). |
...
The application is one deployable unit with an embedded web server (Tomcat, default port 8343). TLS can be terminated in-process by the embedded server or by a reverse proxy in front — `X-Forwarded-Proto` and `X-Forwarded-For` headers are honoured.

Three things run inside the application process:

* **Web UI and REST API** — the interactive application and the integration API.
* **Scheduled jobs** — all integration synchronisation and housekeeping runs as internal scheduled tasks (the most important are shown in the table below). Jobs can be switched off per instance with `SCHEDULING_ENABLED`.
...
The most important jobs are shown below with their default schedules; every schedule is a configurable cron expression.

| Job | Default schedule |
|---|---|
| KITOS delta sync | Every 30 minutes |
| KITOS full re-sync | Daily at 03:00 |
| KITOS deletion sync | Every 30 minutes |
...
* **Object storage (S3):** binary files — report PDFs in the digital signing flow and images uploaded through the rich-text editor. The database only stores the object keys; no files are stored as blobs in the database. Reports that are downloaded or mailed directly are generated on the fly and do not touch S3.
...
Roles are delivered as a SAML claim (claim name and values are configurable); supplementary roles can also be assigned directly on the user in the database. Five coarse roles are mapped to fine-grained rights at login:

| Role (default claim value) | Access |
|---|---|
| `ROLE_administrator` | Full access including administration and settings |
| `ROLE_forandre` | Superuser — create/edit/delete all content, but no access to administration and settings |
